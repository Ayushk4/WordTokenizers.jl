...
function emoticons(ts)
    ...
    idx + 2 <= length(ts.input) && ts[idx] ∈ forehead && (
        ts[idx + 1] ∈ eyes && (
    ...
    ts[idx] == '<' && ts[idx + 1] == '3' && return(flushaboutindex!(ts, idx + 2))

    return false
end

# Stuff to check while dealing with token buffer - out of bounds, return false, return true at the right time to ensure speed

"""
    function emoticonsreverse
